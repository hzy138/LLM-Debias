...
output = "answer_llama3_fassis_with_guidelines_whole.json"
...
with open(output, "a", encoding="utf-8") as f2:
    for sent in tqdm.tqdm(data):
        ...
        # 从数据库中查询guidelines
        guidelines = faiss_db.search_by_question(combined_question, k=1)[0]["law"]
        ...
        messages = [
            ...
            {"role": "user", "content": guidelines + prompt.format(context=sent["context"], question=sent["question"], options=options)}
        ]
